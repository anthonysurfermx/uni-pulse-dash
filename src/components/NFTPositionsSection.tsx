...
export default function NFTPositionsSection({ 
  ...
  const fetchPositions = async () => {
    ...
    try {
      console.log('🎯 Fetching NFT positions for:', walletAddress);
      console.log('📡 Using API:', API_CONFIG.baseURL);
      
      const response = await fetch(
        `${API_CONFIG.baseURL}/api/analyze/${walletAddress}`
      );
      ...
      console.log('📦 NFT Positions data:', result);
    ...
    <div className="mt-10 pt-10 border-t border-gray-800">
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button 
            onClick={() => window.open(`https://app.uniswap.org/positions?account=${walletAddress}`, '_blank')}
            className="px-4 py-2 bg-gradient-to-r from-pink-500 to-purple-600 hover:opacity-90 rounded-lg font-medium text-sm flex items-center gap-2 transition-opacity"
          >
            View All Positions
...
function NFTMiniCard({ position }: { position: NFTPosition }) {
  ...
  const handleCardClick = () => {
    window.open(`https://app.uniswap.org/positions/${position.id}`, '_blank');
  };

  return (
    <div 
      onClick={handleCardClick}
      className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-pink-500/50 hover:shadow-lg hover:shadow-pink-500/10 cursor-pointer"
    >
  ...
  return (
    <div className="text-center py-16 px-8 bg-gray-900/30 rounded-2xl border border-dashed border-gray-800">
      ...
      <p className="text-gray-400 mb-6">
        This wallet doesn't have any Uniswap V3 positions, or provide liquidity on Uniswap V3 to see your positions here.
      </p>
      <button 
        onClick={() => window.open('https://app.uniswap.org/add', '_blank')}
        className="px-6 py-3 bg-gradient-to-r from-pink-500 to-purple-600 hover:opacity-90 rounded-lg font-medium transition-opacity"
      >
